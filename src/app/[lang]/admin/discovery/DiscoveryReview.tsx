...
interface QueueResponse {
  ...
  stats: {
    ...
    approvedTodayInType: number;
    rejectedTodayInType: number;
  };
}
...
export function DiscoveryReview({ lang }: { lang: string }) {
  const isEs = lang === "es";
  ...
  const t = useMemo(
    () => ({
      ...
      pendingConn: isEs ? "Pend. Connectors" : "Pending Connectors",
      pendingSkills: isEs ? "Pend. Skills" : "Pending Skills",
      ...
      summary: isEs ? "Resumen Gemini" : "Gemini summary",
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const res = await authedFetch(`/api/marketplace/admin/discovery/queue?type=${type}&status=${status}`);
  ...
  async function transition(
    ...
    try {
      ...
      if (action === "approve" && overrideCategory) {
        body.marketplace_category = overrideCategory;
      }
      const res = await authedFetch(`/api/marketplace/admin/discovery/${type}/${item.id}`, {
        method: "PATCH",
  ...
  if (auth === "anon")
    return (
      <Banner tone="warn">
        {t.anon}{" "}
        <a href={`/${lang}/login?next=${encodeURIComponent(`/${lang}/admin/discovery`)}`} className="underline">
          {isEs ? "Entrar →" : "Sign in →"}
        </a>
      </Banner>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Stat label={t.approvedToday} value={data.stats.approvedTodayInType} accent="emerald" />
        <Stat label={t.rejectedToday} value={data.stats.rejectedTodayInType} accent="red" />
      </div>
...
function DiscoveryCard({
  ...
  return (
    <li className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)] overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[11px] font-mono text-[var(--color-fg-dim)] truncate">
              {item.product_slug} · {priceLabel}
              {item.creator_handle && <> · {item.creator_handle}</>}
            </p>
          </div>
          <div className={`shrink-0 text-[11px] font-mono ${confColor}`}>
            {(conf * 100).toFixed(0)}%
          </div>
        </div>
      ...
      {(item.raw_title || item.raw_description) && (
        <details className="px-4 sm:px-5 py-3 border-b border-[var(--color-border)]">
          ...
          <div className="mt-2 space-y-1.5 text-[12px] text-[var(--color-fg-muted)]">
            {item.raw_title && <p className="font-semibold">{item.raw_title}</p>}
            {item.raw_description && (
              <pre className="whitespace-pre-wrap font-sans max-h-[200px] overflow-auto">
                {item.raw_description}
              </pre>
            )}
          </div>
      ...
      {/* Category picker (only on pending items) */}
      {status === "pending" && (
          ...
          <p className="text-[10px] font-mono uppercase tracking-[0.12em] text-[var(--color-fg-dim)] mb-2">
            ...
            {item.marketplace_category && (
              <span className="ml-2 normal-case tracking-normal text-[var(--color-accent)]">
                {isEs ? "(Gemini sugiere:" : "(Gemini suggests:"} {item.marketplace_category})
              </span>
            )}
          </p>
          <div className="flex flex-wrap gap-1.5">
      ...
      {status === "pending" ? (
        <div className="grid grid-cols-3 gap-2 p-3 bg-[var(--color-panel-2)]/40">
          ...
          <button
            onClick={() => onTransition("ignore")}
            disabled={busy}
            className="h-11 rounded-xl border border-[var(--color-border)] text-[var(--color-fg-muted)] hover:bg-[var(--color-panel-2)] disabled:opacity-50 text-[13px] font-semibold transition-colors"
          >
            {busy ? "…" : t.ignore}
          </button>
          <button
            onClick={() => onTransition("approve", pickedCategory || undefined)}
            disabled={busy || !pickedCategory}
            className="h-11 rounded-xl text-white bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-[13px] font-semibold transition-colors"
            title={!pickedCategory ? "Picá una categoría primero" : undefined}
          >
            {busy ? "…" : t.approve}
          </button>
        </div>
        ...
        <div className="p-3 text-[12px] text-[var(--color-fg-dim)] bg-[var(--color-panel-2)]/40">
          ...
          {item.marketplace_category && (
            <p className="mt-1 font-mono">→ {item.marketplace_category}</p>
          )}
        </div>
      )}
    </li>
...
function TabPill({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={`px-3 py-2 rounded-lg text-[12.5px] font-semibold transition-colors flex items-center justify-center ${
