      ...
      {snapshot.connectedProviders.map((provider) => (
        <section key={provider.providerId} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)] p-5">
          ...
          <div className="mt-3 text-[12px] leading-relaxed text-[var(--color-fg-muted)]">
            detected_model_ids: {provider.modelIds.map((id) => <CodePill key={id}>{id}</CodePill>).reduce<React.ReactNode[]>((acc, node) => acc.length ? [...acc, " ", node] : [node], [])}
          </div>
          <div className="mt-2 text-[12px] leading-relaxed text-[var(--color-fg-muted)]">
            surfaces: {provider.surfaces.map((surface) => <CodePill key={surface}>{surface}</CodePill>).reduce<React.ReactNode[]>((acc, node) => acc.length ? [...acc, " ", node] : [node], [])}
          </div>
...
export default async function AdminAiCenterPage({ params, searchParams }: Props) {
  ...
  const isEs = lang === "es";
  const snapshot = getAiControlCenterSnapshot();
  const stats = aiCenterStats(snapshot);
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 text-[12px] text-[var(--color-fg-muted)]">
              catalog_status: <CodePill>{snapshot.catalogStatus}</CodePill> · generated_at: {formatDate(snapshot.generatedAt)} · refresh_due: <CodePill>{String(snapshot.refreshDue)}</CodePill>
            </p>
          </div>
