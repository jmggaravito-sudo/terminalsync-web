...
const en: Dict = {
  ...
  comparison: {
    ...
    title: "Terminal Sync vs. the rest",
    subtitle:
      "Stacked against the tools you're probably using today. No tricks, no marketing — if something is partial or not shipped yet, we say so.",
    columns: {
      feature: "Feature",
    ...
    rows: {
      // Persistence
      resurrection: "Session resurrection (close the app, the shell keeps running)",
      uninterruptedWork: "Internet-drop immunity (the agent keeps running locally)",
      backgroundDaemon: "Multiplexer as a background daemon (independent of the UI)",
      // Mobility
      multiDeviceProfile: "Multi-device profile sync (single account)",
      anywhereAccess: "Anywhere Access — open from your phone or any browser",
      pairProgramming: "Read-only pair programming (revocable shared session)",
      // Security
      aes256: "AES-256-GCM zero-knowledge encryption",
      secretsVault: "Encrypted secrets vault (.env per folder)",
      apiKeysKeychain: "API keys in OS Keychain (never on plain disk)",
      // AI native
      aiContinuity: "Claude/Codex continuity (the conversation survives)",
      aiConfigSync: "AI config sync (skills, MCPs, ~/.claude)",
      oneClickInstall: "1-click install of Claude Code + Codex",
      // Productivity
      multiCloudSync: "Encrypted multi-cloud sync (Drive / iCloud / Dropbox)",
      silenceNotifications: "AI-aware notifications (Email / WhatsApp / Telegram)",
      setupOnArrival: "Setup-on-arrival scripts per session",
      gitNativeSync: "Git-native sync per session (auto-snapshots)",
    },
    upcomingLabel: "Coming soon",
