...
6. **I18n gate before PR:** open both `content/connectors/en/<slug>.md` and `content/connectors/es/<slug>.md`; app-visible fields (`name`, `tagline`, subtitles, body/description, setup copy) must be localized, not copied/fallbacked across languages.
...
- `--items`: landing slugs for the connectors added/promoted, so `/admin/integraciones/loop-runs` can show direct `/es/connectors/<slug>` links.
