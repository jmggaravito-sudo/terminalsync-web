...
| **meta-ads** | FIRST-PARTY 2026-07-19 | NO pasa por el Loop npm (Meta no publica un MCP operador para el dueño; los npm son de terceros). Build propio: server `terminalsync-meta-ads-mcp` (repo `terminal-sync`, sidecar Tauri) **solo lectura** sobre la Marketing (Graph) API oficial, conectado desde Ajustes → Integraciones. Catálogo: `content/connectors/{es,en}/meta-ads.md` (`source: first-party`, sin manifest). Cierra el gap del trabajo automatizado "Vigía de campañas". **Un futuro run del Loop NO debe re-SKIPpearlo como npm.** Logo: oficial (simple-icons `meta`, `#0467DF`). Auth: token hoy; OAuth "Conectá con Facebook" para GA. |
