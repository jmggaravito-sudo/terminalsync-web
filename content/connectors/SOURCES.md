...
| **shopify** | SKIP-diferido 2026-07-19 | Persona `business` (ventas/e-commerce, alta prioridad) pero **ningún camino oficial pasa el gate hoy**. (1) El único paquete **oficial en npm** es `@shopify/dev-mcp` (publisher `shopify-admin`/`shopify-dep`), y es una herramienta **para desarrolladores**: sus tools son `learn_shopify_api`, `introspect_admin_schema`, `search_docs`, `validate_graphql_codeblocks` — introspecciona/valida el **schema** de la Admin API, NO opera la tienda (no lista órdenes ni gestiona productos). Falla el filtro de persona: un dueño no puede preguntar *"¿cuánto vendí?"*. (2) Los paquetes npm que SÍ operan la tienda (`shopify-mcp` de geli2001, `shopify-mcp-server` de amir-bengherbi) son **de terceros** → fallan el gate de publisher oficial (regla #3). (3) La oferta **operadora oficial de Shopify es REMOTA/hospedada** (mismo caso que `cloudflare`): Storefront MCP en `https://{shop}.myshopify.com/api/mcp` (transporte http, `claude mcp add --transport http`) y Customer Accounts MCP (OAuth PKCE) — pero ambos son **shopper-facing** (buscar catálogo, carrito, estado de pedido del cliente), no operación del dueño. Un MCP **Admin operador** oficial para el dueño no es verificable desde `shopify.dev` en este entorno (403 vía WebFetch). **Camino:** cuando el catálogo modele conectores remotos con transporte http nativo (hay precedente: `posthog` vía bridge `mcp-remote`), retomar en una corrida CON acceso a `shopify.dev` para fijar endpoint + scopes + tools del server operador oficial desde la fuente. Alternativa de mayor foso: build propio contra la Admin GraphQL API oficial (mismo patrón que el conector Mercado Pago), fuera del scope del Loop npm. |
