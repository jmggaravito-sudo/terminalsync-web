...
| ~~**Zoom**~~ | **CERRADO — SHIPPED 2026-07-24** (ver fila `zoom` arriba). Se pudo sourcear porque Zoom publica su README oficial en `github.com/zoom/mcp-registry` (raw GitHub, no bloqueado), a diferencia de Asana/Klaviyo/Canva cuyos dev-portals dan 403. | OAuth (molde B) | business |

Los leads de Asana/Klaviyo/Canva salen de WebSearch de páginas oficiales; **NO alcanzan para redactar** (regla #1 exige leer la fuente) y sus dev-portals bloquean el fetcher (403). Cada uno se cierra cuando una corrida pueda fetchear el dev-portal del vendor, O si el vendor publica su README/registry en GitHub (como hizo Zoom). Prioridad: **Asana** (endpoint V2 confirmado + GA).
...
|---|---|---|---|
| **ideogram** | Fallback TS | Conseguir logo oficial de Ideogram y vendorizarlo en `public/connectors/ideogram.svg`. | 2026-07-29 |
...
| **zapier** | Fallback TS | Reemplazar por logo oficial de Zapier/simple-icons si está disponible en la próxima corrida. | 2026-07-29 |
