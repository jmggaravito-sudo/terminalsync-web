...
Le preguntas *"¿cuántos usuarios se registraron la semana pasada?"* y genera la query SQL, la corre y te entrega el número. Le pedís *"créame una tabla nueva para tracking de invitaciones"* y prepara el migration SQL — *"SQL passed to this tool will be tracked within the database, so LLMs should use this for DDL operations"*, según el README oficial.

### Qué le puedes pedir

- *"Lístame las tablas del schema `public` con sus columnas y tipos — necesito entender la estructura antes de hacer un análisis."*
- *"Cuántos usuarios activos tuvimos en los últimos 30 días, agrupados por país."*
- *"Genera la migration para agregar una columna `subscription_tier` a la tabla `users` con default 'free'."*
...
Para activarlo, pásale el flag `--read-only` al server o usá un Postgres role read-only en tu proyecto. Ideal para dashboards, análisis y debugging; activa el modo write solo en proyectos de desarrollo.
